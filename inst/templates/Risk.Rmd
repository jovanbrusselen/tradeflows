```{r productname, results='asis', eval=TRUE}

productname <- tfdata %>% 
    select(classification, productdescription) %>%
    arrange(classification) %>%
    unique
productname[productname=='H1'] <- '1'
productname[productname=='H2'] <- '2'
productname[productname=='H3'] <- '3'
productname[productname=='H4'] <- '4'
productname[productname=='H5'] <- '5'
productname[productname=='H6'] <- '6'

name <- productname$productdescription[productname$classification == max(productname$classification)]
...
---
title: "Forest Products Trade Risk - Trade by `r query[[1]]` in HS `r query[[2]]` - `r name`"
...
## Product definitions
For the given time period, countries reported data for product `r query[[2]]` according to the following definitions. When more than one definition is stated, this means that not all countries report their data according the latest classification and this implies that data compatibility issues need to be taken into account with data interpretation.
...
productname <- tfdata %>% 
    ...
    arrange(classification) %>%
    unique
productname[productname=='H1'] <- 'HS v. 1992'
productname[productname=='H2'] <- 'HS v. 1996'
productname[productname=='H3'] <- 'HS v. 2002'
productname[productname=='H4'] <- 'HS v. 2007'
productname[productname=='H5'] <- 'HS v. 2012'
productname[productname=='H6'] <- 'HS v. 2017'

for(code in productname$classification){
    cat("\n\n__",code,":__ ", sep="")
    ## DEBUG_RAUL:
        # Previous code: cat(productname$product[productname$classification == code])
        # There is no such column called "product" in the productname data frame, so I changed it to product description.
    cat(productname$productdescription[productname$classification == code])
}
```
